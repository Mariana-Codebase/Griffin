...
│   ├── pyproject.toml
│   └── src/
│       ├── api.py              # FastAPI: POST /audits · GET /state · GET /report
...
// check.valid === false — amount doesn't match stated intent
...
listener.on("threat", (t) => {
  console.log(`Known exploit active: ${t.vector_id} (CVSS ${t.cvss / 10})`)
})
```
...
Download as PDF via **Download Report** button — `window.print()` with full print-optimized CSS for clean A4 output.
...
| Voice | ElevenLabs (5 distinct attacker voices) |
